...
public class uteMassBuildEngine : MonoBehaviour {
	...
	public IEnumerator AddTile(GameObject obj)
	{
		...
		if(isItFirst)
		{
			...
			newDummy.transform.localScale = new Vector3(obj.collider.bounds.size.x,0.1f,obj.collider.bounds.size.z);
		...
		if(isGoodToGo)
		{
			...
			if(newDistance!=lastDistance)
			{
				...
				int Xfill = (int) (Mathf.Abs(Xdistance)/obj.collider.bounds.size.x);
				int Zfill = (int) (Mathf.Abs(Zdistance)/obj.collider.bounds.size.z);
				string pM = "";
				...
				for(int i=0;i<Mathf.Abs(Xfill)+1;i++)
				{
					for(int j=0;j<Mathf.Abs(Zfill)+1;j++)
					{
						...
						newTiles.Add(new Tile(position,obj.transform.localScale,obj.collider));
